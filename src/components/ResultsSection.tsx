...
const ResultsSection = () => {
  ...
  return <section id="results" className="py-20 bg-gray-50">
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          loop: true
            ...
            <CarouselContent className="-ml-2 md:-ml-4">
              {results.map((result, index) => <CarouselItem key={index} className="pl-2 md:pl-4 md:basis-1/2 lg:basis-1/3">
                  <div className="bg-white rounded-lg shadow-md p-8 hover:shadow-lg transition-all duration-300 border border-gray-100 h-full">
                    <div className="flex justify-between items-center mb-6">
                      <div className="flex flex-col items-center">
                        {result.logo ? <div className="h-28 w-48 flex items-center justify-center mb-2">
                            <img src={result.logo} alt={`${result.company} logo`} className="max-h-full max-w-full object-contain" />
                          </div> : <div className="h-28 w-48 flex items-center justify-center mb-2">
          ...
          <div className="flex justify-center mt-8 space-x-2">
            {results.map((_, index) => <div key={index} className="w-2 h-2 bg-consultant-gray-300 rounded-full"></div>)}
          </div>
        </div>
      </div>
    </section>;
